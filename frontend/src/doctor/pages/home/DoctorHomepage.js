...
const DoctorHomepage = () => {
  ...
  const getStatCard = (title, subtitle, color) => {
    return (
      <div className={classes.statCard} style={{ backgroundColor: color }}>
  ...
  const getPatientTile = (patient) => {
    ...
    return (
      <div className={classes.patientTile}>
        <img src={defaultProfileImg} />
        <div className="ms-3" style={{ flex: 1 }}>
          <div className={classes.patientName}>{patient.name}</div>
          <div className={classes.patientDescription}>
            {patient.gender}, {age}, {patient.mobileNumber}{' '}
          </div>
        </div>
        <div onClick={toPatient} className={classes.patientButton}>
          View
        </div>
      </div>
    );
        ...
        <div className="d-flex justify-content-between w-100 px-5">
          <div className="col-4">
            ...
            <div className={classes.appointmentContainer}>
              {doctor != null &&
                doctor.patients.map((patient) => getPatientTile(patient))}
            </div>
          </div>
          <div>
            {currentAppointment && (
          ...
          <div className="col-3">
            ...
            <div className={classes.appointmentContainer}>
              {appointments != null &&
                appointments.map((app) => getAppointmentTile(app))}
            </div>
          </div>
        </div>
      </div>
