...
const PrescriptionTile = (props) => {
    ...
    return <div onClick={toggleCheck} className={classes.tableRow}>
        <div className={classes.checkbox}><Checkbox checked={checked} size="small" /></div>
        ...
        <div style={{ flex: 1 }} className={`${classes.statusPill} ${presc.status == "Filled" ? classes.statusFilled : classes.statusUnfilled}`}>{presc.status}</div>
    </div>
}
