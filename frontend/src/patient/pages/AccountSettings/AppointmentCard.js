...
const AppointmentsCard = (props) => {
    ...
    useEffect(() => {
      ...
      let newAppointments = allAppointments.filter(app => app.status == tabText || (tab == 0 && app.status == "Rescheduled"));
      if (pickedDate != null) {
    ...
    const appsByTab = () => {
      ...
      return allAppointments.filter(app => app.status == tabText || (tab == 0 && app.status == "Rescheduled"));
    }
    ...
    return <SideCard>
      <div className='d-flex position-relative justify-content-between align-items-center'>
        ...
        <div className={classes.dateSelection}>
          {pickedDate != null && <span className='d-flex align-items-center me-3'>
            {formatDate(pickedDate)}, {pickedTime.label} 
            <img className='ms-2' onClick={() => setPickedDate(null)} height={12} src={closeImg}/> 
            </span>}
          <img className={classes.iconButton} onClick={() => {setShowCalendar(val => !val)}} src={calendarImg}/>
        </div>
        {showCalendar && <div className={classes.calendarContainer}>
        <Calendar className={classes.calendar} value={pickedDate} onChange={(value) => setPickedDate(value)}/>
      ...
      <div className={classes.appWrapper}>
        {appointments.map(app => {
          return <div key={app.date} className={classes.appContainer}>
            <div className='d-flex mb-2'>
              ...
              <div className='d-flex flex-column ps-3 py-1 justify-content-between'>
                ...
                <div className={classes.appDescription}>{formatDateTime(app.date)}</div>
              </div>
            </div>
