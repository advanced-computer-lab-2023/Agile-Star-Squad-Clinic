...
const fetchMobileNumber = () =>{
    fetch(`http://localhost:3000/patients/${patient.userId}`, {
        ...
        const json = await response.json();
          setMobileNumber(json.data.patient.mobileNumber);
    
      });
}
useEffect(()=>{fetchMobileNumber()},[]);
...
const fetchGender = () =>{
    fetch(`http://localhost:3000/patients/${patient.userId}`, {
        ...
        const json = await response.json();
          setGender(json.data.patient.gender);
...
const handleSubmit = async (e) => {
      ...
      if (isPasswordValid()){
        const response = await axios
          .patch(`http://localhost:3000/auth/resetPassword/${patient.userId}`, {
            newPassword,
          })
          .catch((err) => {
            console.log(err);
