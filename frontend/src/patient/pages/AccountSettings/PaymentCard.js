...
const PaymentCard = (props) => {
  ...
  const onSave = () => {
    ...
    if (isAdding) {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        if (currCard.cardNumber === selectedCard.cardNumber 
          && currCard.name === selectedCard.name 
          && currCard.cvc === selectedCard.cvc 
  ...
  const onDelete = () => {
    setSavedCards(cards => {
      return cards.filter(currCard => !(currCard.cardNumber === selectedCard.cardNumber 
        && currCard.name === selectedCard.name 
        && currCard.cvc === selectedCard.cvc 
        && currCard.expiryMonth === selectedCard.expiryMonth 
        && currCard.expiryYear === selectedCard.expiryYear))
  ...
  return <SideCard>
    ...
    <input value={name} onChange={(e) => setName(e.target.value)} className={classes.input} />
    <div className={classes.inputLabel}>Card Number</div>
    <input value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} className={classes.input} />
    <div className="d-flex mb-3">
      <div>
        <div className={classes.inputLabel}>Expiration Date</div>
        <div className="d-flex align-items-center">
          <input value={expiryMonth} onChange={(e) => setExpiryMonth(e.target.value)} className={`${classes.input} w-25 me-2`} />
          /
          <input value={expiryYear} onChange={(e) => setExpiryYear(e.target.value)} className={`${classes.input} w-25 ms-2`} />
        </div>
      </div>
      <div>
        <div className={classes.inputLabel}>CVC</div>
        <div className="d-flex align-items-center"><input value={cvc} onChange={(e) => setCvc(e.target.value)} className={`${classes.input} w-50`} /></div>
      </div>
    </div>
    <div className="d-flex justify-content-between">
      {!isAdding && <button onClick={onDelete} className={classes.deleteButton}>Delete</button>}
      <div />
