...
const PatientHome = () => {
      ...
      switch (prescriptionFilter) {
        case 'filled':
          newPrescriptions = prescriptions.filter(
            (presc) => presc.status == 'Filled'
          );
          setFilteredPrescriptions(newPrescriptions);
          break;
        case 'unfilled':
          newPrescriptions = prescriptions.filter(
            (presc) => presc.status == 'Unfilled'
          );
          setFilteredPrescriptions(newPrescriptions);
          break;
        case 'date':
  ...
  useEffect(() => {
    ...
    doctorSearchNameValue,
    doctorSearchSpecialtyValue,
    doctorSearchDateValue,
  ...
  const fetchDoctors = async (patientDiscount) => {
    fetch('http://localhost:3000/doctors/').then(async (response) => {
      ...
      setSpecialtyFilters(
        specialtyList.map((spec) => <option value={spec}>{spec}</option>)
      );
    });
  };
  ...
  const prescDoctorDropdownHandler = (event) => {
    ...
    if (chosenDoctor === 'Select') {
      ...
      const newPrescriptions = prescriptions.filter(
        (presc) => presc['doctorName'] === chosenDoctor
      );
      setFilteredPrescriptions(newPrescriptions);
    }
  };
  ...
  const applySearch = () => {
    ...
    if (doctorSearchNameValue !== '') {
      newDoctors = newDoctors.filter((doc) =>
        doc.name.toLowerCase().includes(doctorSearchNameValue)
      );
    }
    if (doctorSearchSpecialtyValue !== '') {
      newDoctors = newDoctors.filter((doc) =>
        doc.speciality.toLowerCase().includes(doctorSearchSpecialtyValue)
      );
    }
    if (doctorSpecialtyFilter !== 'Select') {
      newDoctors = newDoctors.filter(
        (doc) => doc.speciality === doctorSpecialtyFilter
      );
    }
    ...
    <div className="center">
      ...
      <span>
        ...
        <select
          ...
          onChange={prescriptionDropdownHandler}
        >
          {prescriptionFilters}
        </select>
        {showPrescDateFilter && (
          <input
            ...
            onChange={prescDateFilterHandler}
          />
        )}

        {showPrescDoctorFilter && (
          <select
            value={prescDoctorFilter}
            onChange={prescDoctorDropdownHandler}
          >
      ...
      <span>
        ...
        <div>
          ...
          <input
            ...
            onChange={(event) => {
              setDoctorSearchName(event.target.value);
            }}
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(event) => {
              setDoctorSearchDate(event.target.value);
            }}
          />
        ...
        <div>
          ...
          <select
            ...
            onChange={doctorSpecialtyDropdownHandler}
          >
            {specialtyFilters}
          </select>
          <button onClick={() => setDoctorSpecialtyFilter('Select')}>
            Cancel
          </button>
        </div>
