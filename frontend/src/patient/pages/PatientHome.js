...
const PatientHome = () => {
  ...
  const fetchPrescriptions = () => {
    fetch(
      `http://localhost:3000/patients/${DUMMY_USER._id}/prescriptions`
    ).then(async (response) => {
      const json = await response.json();
  ...
  const fetchUpcomingAppointments = () => {
    fetch(
      `http://localhost:3000/patients/${DUMMY_USER._id}/upcomingAppointments`
    ).then(async (response) => {
      const json = await response.json();
  ...
  const prescDoctorDropdownHandler = (event) => {
    ...
    const newPrescriptions = prescriptions.filter(
      (presc) => presc['doctor'] === chosenDoctor
    );
    setFilteredPrescriptions(newPrescriptions);
  };
